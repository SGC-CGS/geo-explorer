export default class Style {
	...
	static Classes(data, n) {
        // sort data in numerical order
        data = data.slice().sort(function (a, b) { return a - b; });

        // get our basic matrices
        var matrices = Style.Matrices(data, n),
    ...
    // can be used for any classing of data with `classes <= n`
	static Matrices(data, n) {
	...
	static ClassBreaks(classes, symbols) {
		var classBreaks = [];
		
		for (var i = 1; i < classes.length; i++) {
			classBreaks.push({
				minValue: classes[i - 1],  
				maxValue: classes[i],
				symbol: symbols[i - 1]
			});
		}
